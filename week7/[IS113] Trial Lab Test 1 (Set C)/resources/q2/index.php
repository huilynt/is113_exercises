...
if (isset($_GET['horoscope'])) {
   ...
   $is_valid = true;
   if (!array_key_exists($horoscope, $horoscopeDict) || $horoscope === '') {
      $is_valid = false;
   }
}
?>
...
<body>
   ...
   <form action="index.php">
      Horoscope
      <input type="text" name='horoscope' value='<?php echo $horoscope; ?>'>
      <input type="submit" value="Submit">
   </form>
   <?php
   if (isset($horoscope)) {
      if ($is_valid) {
         $luck_arr = ['GOOD', 'BAD', 'NORMAL'];
         $result = luckStringToDict($horoscopeDict[$horoscope]);
         foreach ($luck_arr as $luck) {
            echo "<b>$luck:&nbsp;</b>";
            if (array_key_exists($luck, $result)) {
               $months = implode(', ', $result[$luck]);
            } else {
               $months = 'NIL';
            }
            echo "$months<br>";
